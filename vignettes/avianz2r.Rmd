...
# convert the timestamps to relative times from last sunset
head(difftime(annots$tstart, lubridate::ymd_hms("20180122 204900"), units="mins"))
```
...
The main output of interest here is the call density estimate of `r round(coef(cr)["D"], 3)` calls/ha/survey. The estimated detection area is high for a typical bird survey, as this example dataset had many calls captured on all 3 recorders. 
...
Leading to a final density estimate of `r round(cr$coefficients["Da"], 3)` animals/ha.
